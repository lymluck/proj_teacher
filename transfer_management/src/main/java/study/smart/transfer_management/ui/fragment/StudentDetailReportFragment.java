...
import study.smart.transfer_management.mvp.presenter.StudentDetailReportPresenter;
 ...
 */
public class StudentDetailReportFragment extends UIFragment<StudentDetailReportContract.Presenter> implements StudentDetailReportContract.View {
    ...
    public static StudentDetailReportFragment getInstance(Bundle bundle) {
        StudentDetailReportFragment fragment = new StudentDetailReportFragment();
    ...
    public void onFirstUserVisible() {
        ...
        presenter.getStudentDetailReport(userId, stateInfo.getType(), mPage + "", ParameterUtils.PULL_DOWN);
    }
    ...
    @Override
    public StudentDetailReportContract.Presenter initPresenter() {
        return new StudentDetailReportPresenter(this);
    }
    ...
    @Override
    public void getStudentDetailReportSuccess(List<MyStudentInfo> myStudentInfos, int request_state) {
        if (presenter != null) {
    ...
    private void initAdapter() {
        ...
        mAdapter = new CommonAdapter<MyStudentInfo>(mActivity, R.layout.item_student_detail_report, myStudentInfos) {
            @Override
            protected void convert(ViewHolder holder, MyStudentInfo myStudentInfo, int position) {
                if ("QUARTERLY".equals(stateInfo.getType())) {
                    holder.setText(R.id.tv_content, myStudentInfo.getTime() + "报告");
                } else {
                    holder.setText(R.id.tv_content, myStudentInfo.getTypeTEXT());
                }
                holder.setText(R.id.tv_time, TimeUtil.getStrTime(myStudentInfo.getPublishTime()));
            }
        };
        ...
        lmrvReport.SetOnLoadMoreLister(new LoadMoreRecyclerView.OnLoadMoreListener() {
            ...
            public void OnLoad() {
                ...
                mPage = mPage + 1;
                presenter.getStudentDetailReport(userId, stateInfo.getType(), mPage + "", ParameterUtils.PULL_UP);
            }
        });
